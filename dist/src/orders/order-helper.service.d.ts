...
export declare class OrderHelperService {
    ...
    saveOrder(dto: CreateOrderDto, userId: string, subTotal: number, discount: number, shippingCharge: number, estimatedTax: number, totalAmount: number): Promise<Order>;
    ...
    handlePaypalPayment(order: Order, totalAmount: number, res: Response, payload?: JWTPayloadType, accessToken?: string, refreshToken?: string): Promise<{
        message: string;
        success: boolean;
        data: {
            order: Order;
            paypalApprovalUrl: any;
        };
    }>;
}
